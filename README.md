...
当前版本可以稳定支持“售前人员转述客户需求 -> 多轮需求补齐 -> 证据检索 -> 能力匹配 -> 方案生成 -> 风险校验 -> 最终安全方案输出”的完整闭环。
...
| 风险校验 Agent | 检查过度承诺、支持等级错误、系统依赖遗漏和人工确认项遗漏，并输出最终安全文本。 | `risk_check_result`<br>`final_safe_response` |
...
MVP 阶段放入同一个知识库，便于证据检索 Agent 对以下资料统一检索：
...
| 评估目录 | 覆盖内容 | 当前结果 |
| --- | --- | --- |
| `requirement_clarifier/` | 信息缺失追问、完整需求路由、结构化输出、多轮状态合并。 | 已完成 |
| `evidence_retrieval/` | 逐项证据整理、引用来源、限制条件、证据缺口。 | 已完成 |
| `capability_matching/` | 支持等级、交付边界、人工确认项和风险标记。 | 已完成 |
| `solution_generation/` | 基于能力匹配结果生成售前初版方案，避免新增未验证能力。 | 已完成 |
| `risk_check/` | 检查并修正过度承诺、系统依赖遗漏、私有化部署边界和人工确认项。 | 已完成 |
| `e2e_chatflow/` | 记录完整多轮 Chatflow 的端到端用例与运行结果。 | 5 条 E2E 全部 Pass |
...
| Case ID | 测试目标                                                     | 结果 |
| ------- | ------------------------------------------------------------ | ---- |
| E2E-001 | 售前内部输入主链路：多轮补齐后完成方案生成与风险校验。       | Pass |
| E2E-002 | 信息不足时必须阻断，不得假设需求或生成方案。                 | Pass |
| E2E-003 | 明确不需要系统对接时，不得强行生成 API 依赖。                | Pass |
| E2E-004 | 外部数据查询必须保留 API 对接边界。                          | Pass |
| E2E-005 | 不支持能力、自动报价、明确交付周期等高风险诉求必须保留边界。 | Pass |

端到端测试证明当前 Chatflow 能够支持以下完整流程：
...
- 从单点 Agent 到完整端到端 Chatflow 的渐进式验证方法。
...
- 扩展知识库覆盖范围，例如公有云部署、SLA、报价流程、行业案例等。
- 增加更多行业场景，例如制造、金融、政企、教育等。
- 增加更复杂的会议纪要输入测试。
- 增加批量需求项、多客户角色、多系统对接场景。
- 引入自动化 Eval 脚本，对结构化输出进行批量断言。
- 增加 Dify 工作流导出文件和部署说明。
- 优化最终方案的销售表达质量和可复用性。
