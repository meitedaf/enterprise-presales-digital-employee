...
本项目设计一个基于 Dify Chatflow 的售前方案生成 Agent，用于模拟企业售前数字员工的核心工作流：将客户模糊需求结构化，基于企业知识库检索证据，评估产品能力匹配关系，生成可供售前人员审核的初版方案，并标记风险和人工确认项。

该项目的定位不是直接替代售前人员做最终承诺，而是提升售前前期响应效率，降低资料查找、方案起草和能力边界遗漏的成本。
...
1. 客户需求表达不完整，售前需要反复追问应用场景、目标用户、部署方式、系统对接和交付约束。
...
1. 将客户原始需求转化为结构化需求，并在信息不足时生成追问问题。
...
```text
客户希望建设一个 AI 客服系统，用于处理商品咨询、订单查询、退换货政策问答，并要求支持转人工和私有化部署。
```
...
最终输出定位为“售前可审核的初版方案”，不作为面向客户的正式承诺。当前 Dify Chatflow 中的直接回复节点仅用于调试中间结果，例如 evidence_results 的 JSON 输出；正式产品中应在能力匹配评估、方案生成和风险校验后再输出面向售前人员的汇总结果。

## 8. 处理边界与人工确认

### 8.1 需求信息不足

当应用场景、目标用户、核心功能需求、部署方式或系统对接信息缺失时，流程应暂停在需求梳理阶段，输出追问问题，不进入证据检索和方案生成。用户下一轮补充后，系统应读取已保存的 requirement_state_text、missing_fields_state 和 last_route_decision，将历史已确认信息与本轮输入合并后重新判断。
### 8.2.1 证据归因边界
...
2. 追问合理性  
   信息不足时，Agent 是否能识别缺失字段，并生成面向客户的有效追问。
